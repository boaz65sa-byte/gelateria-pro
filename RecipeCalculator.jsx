...
const navGroups = [
  ...
  },
  {
    label: 'תפריט',
    items: [
      { path:'/menu',    label:'Sweet Station', sub:'תפריט דו-לשוני',  icon:'IceCream'  },
  ...
  },
  {
    ...
    items: [
      ...
      { path:'/custom',  label:'Dal sacchetto', sub:'מתכון מהשקית',    icon:'Recipe'    },
    ]
  },
  {
    label: 'מלאי ודוחות',
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {navGroups.map((group, gi) => (
            <div key={gi} className={gi > 0 ? 'mt-5' : ''}>
              <p className="px-3 mb-1.5 font-sans font-semibold text-espresso-400 dark:text-espresso-400"
                      ...
                      <NavLink to={item.path} end={item.path === '/'} onClick={onClose}
                        className={({ isActive }) =>
                          `group flex items-center gap-3.5 px-3.5 py-2.5 rounded-xl transition-all duration-150 ${
                            isActive ? 'bg-terra-400 text-white' : 'text-espresso-500 dark:text-espresso-200 hover:bg-canvas dark:hover:bg-espresso-700'
                          }`
                        }>
                        {({ isActive }) => (
                          <>
                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${
                              isActive ? 'bg-white/20 text-white' : 'bg-canvas dark:bg-espresso-700 text-espresso-400 dark:text-espresso-300 group-hover:bg-silk dark:group-hover:bg-espresso-600'
                            }`}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className={`text-xs leading-tight truncate font-sans ${isActive ? 'text-white/70' : 'text-espresso-400 dark:text-espresso-400'}`}>{item.sub}</p>
                            </div>
                          </>
                        )}
                      </NavLink>
